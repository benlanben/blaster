...
{
	if (CarriedAmmoMap.Contains(WeaponType) && CarriedAmmoMap[WeaponType] < MaxCarriedAmmo)
	{
...
void UCombatComponent::OnRep_CarriedAmmo()
{
	...
	bool bJumpToShotgonEnd = CombatState == ECombatState::ECS_Reloading && EquippedWeapon && 
		EquippedWeapon->GetWeaponType() == EWeaponType::EWT_Shotgun && CarriedAmmo == 0;
	if (bJumpToShotgonEnd)
	{
		JumpToShotgunEnd();
	}
}
...
{
	if (!Character || !Character->GetMesh() || !Flag || !EquippedWeapon) return;
...
void UCombatComponent::AttachSecondaryWeapon(AActor* ActorToAttach)
{
	if (!Character || !Character->GetMesh() || !ActorToAttach) return;
	
	if (AWeapon* WeaponToAttach = Cast<AWeapon>(ActorToAttach))
	{
		FName SocketName;
		switch (WeaponToAttach->GetWeaponType())
		{
		case EWeaponType::EWT_AssaultRifle:
			SocketName = FName("ARSniperSocket");
			break;
		case EWeaponType::EWT_RocketLauncher:
			SocketName = FName("RocketLauncherSocket");
			break;
		case EWeaponType::EWT_Pistol:
			SocketName = FName("PistolSMGSocket");
			break;
		case EWeaponType::EWT_SubmachineGun:
			SocketName = FName("PistolSMGSocket");
			break;
		case EWeaponType::EWT_Shotgun:
			SocketName = FName("ShotgunGLSocket");
			break;
		...
		if (const USkeletalMeshSocket* SecondaryWeaponSocket = Character->GetMesh()->GetSocketByName(SocketName))
		{
...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (Controller)
	{
		...
		if (HUD)
		{
			...
			else if (CrosshairInAirFactor != 0.f)
			{
			...
			else if (CrosshairAimFactor != 0.f)
			{
				CrosshairAimFactor = FMath::FInterpTo(CrosshairAimFactor, 0.f, DeltaTime, 30.f);
			}

			if (CrosshairShootingFactor != 0.f)
			{
...
void UCombatComponent::InterpFOV(float DeltaTime)
{
	...
	if (bAiming && CurrentFOV != EquippedWeapon->GetZoomedFOV())
	{
		CurrentFOV = FMath::FInterpTo(CurrentFOV, EquippedWeapon->GetZoomedFOV(), DeltaTime, EquippedWeapon->GetZoomInterpSpeed());
	}
	else if (!bAiming && CurrentFOV != DefaultFOV)
	{
